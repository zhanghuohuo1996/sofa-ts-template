...
import { updateRepeatPassword } from '../App/actions';
    ...
    return (
      <PanelItem>
        <Input
          type="password"
          size="large"
          placeholder={formatMessage(messages.confirmNewPwd)}
          prefix={<Icon type="check" style={{ color: 'rgba(0,0,0,.25)', fontSize: '16px' }} />}
          suffix={suffix}
          value={password}
...
const mapStateToProps = createStructuredSelector({
  password: makeSelectRepeatPassword(),
});

export function mapDispatchToProps(dispatch) {
  return {
    updatePassword: (data) => dispatch(updateRepeatPassword(data)),
  };
}
