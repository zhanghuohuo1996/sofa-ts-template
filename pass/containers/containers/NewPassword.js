...
import { makeSelectNewPassword } from '../App/selectors';
import { updateNewPassword } from '../App/actions';
    ...
    return (
      <PanelItem>
        <Input
          type="password"
          size="large"
          placeholder={formatMessage(messages.newPwd)}
          prefix={<Icon type="lock" style={{ color: 'rgba(0,0,0,.25)', fontSize: '16px' }} />}
          suffix={suffix}
          value={password}
...
const mapStateToProps = createStructuredSelector({
  password: makeSelectNewPassword(),
});

export function mapDispatchToProps(dispatch) {
  return {
    updatePassword: (data) => dispatch(updateNewPassword(data)),
  };
}
